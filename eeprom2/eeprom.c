...
uint16_t EE_Init(void)
{
  ...
  cdcprintf("EEPROM_START=0x%x\r\n", EEPROM_START_ADDRESS);
  cdcprintf("PAGE0_BASE_ADDRESS=0x%x\r\n", PAGE0_BASE_ADDRESS);
  cdcprintf("PAGE0_END_ADDRESS=0x%x\r\n", PAGE0_END_ADDRESS);
  cdcprintf("PAGE1_BASE_ADDRESS=0x%x\r\n", PAGE1_BASE_ADDRESS);
  cdcprintf("PAGE1_END_ADDRESS=0x%x\r\n", PAGE1_END_ADDRESS);
  ...
  switch (PageStatus0)
  {
    case ERASED:
      ...
      if (PageStatus1 == VALID_PAGE) /* Page0 erased, Page1 valid */
      {
        ...
        /* Erase Page0 */
        FlashStatus = FLASH_ErasePage(PAGE0_BASE_ADDRESS); //smooker
...
uint16_t EE_ReadVariable(uint16_t VirtAddress, uint16_t* Data)
{
  ...
  uint32_t Address = 0x0800F800, PageStartAddress = 0x0800F800;     //smooker
...
static FLASH_Status EE_Format(void)
{
  cdcprintf("EE_Format() 0\r\n");
  FLASH_Status FlashStatus = FLASH_COMPLETE;

  /* Erase Page0 */
  FlashStatus = FLASH_ErasePage(PAGE0_BASE_ADDRESS);
  cdcprintf("EE_Format() 1:%x\r\n", FlashStatus);
...
static uint16_t EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data)
{
  ...
  uint32_t Address = 0x0800F800, PageEndAddress = 0x0800FFFF;     //smooker 0x0800F7FF
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  cdcprintf("EE_PageTransfer() 0:%x\r\n", VirtAddress);
  FLASH_Status FlashStatus = FLASH_COMPLETE;
  // uint32_t NewPageAddress = 0x080103FF, OldPageAddress = 0x08010000;
  uint32_t NewPageAddress = 0x0800FCFF, OldPageAddress = 0x0800F000;     //smooker
